  ...
  if (
    ...
    $.ajax({
      ...
      url:
        ...
        '&lon=' +
        geo_info_object.lon +
        '&units=metric&appid=b231606340553d9174136f7f083904b3',
      dataType: 'json',
      success: function(data) {
        ...
        switch (data['weather'][0]['description']) {
          case 'broken clouds':
            $('#weatherIcon').attr(
              'src',
              'images/weather_icon/sun-rays-cloud.png'
            );
            break;
          case 'clear sky':
            $('#weatherIcon').attr(
              'src',
              'images/weather_icon/sun-rays-small.png'
            );
            break;
          ...
          case 'thunderstorm':
            $('#weatherIcon').attr(
              'src',
              'images/weather_icon/cloud-dark-multiple-lightning.png'
            );
            break;
          case 'snow':
            $('#weatherIcon').attr(
              'src',
              'images/weather_icon/cloud-dark-snow.png'
            );
            break;
          case 'mist':
      ...
      error: function() {
        $('.data')
          .text('Sorry, your temperature info is missing!')
          .addClass('displayTempError');
      }
    });
  } else {
    $('#weatherIcon').attr('src', 'images/weather_icon/sadFace.png');
...
function weatherOutputWithData() {
  ...
  $('#weatherCurrent').append(
    'Current Temperature: ' + parseInt(geo_info_object.temperature) + '&deg; C'
  );
  $('#weatherTemp').append(
    'Temperature Range: ' +
      geo_info_object.minTemp +
      '&deg; C' +
      ' - ' +
      geo_info_object.maxTemp +
      '&deg; C'
  );
...
function weatherOutputWithoutData() {
  ...
  $('#weatherIcon').empty();
  $('#weatherCity').append(
    'Sorry, no weather info available for entered location.  Please enter another city.'
  );
}
...
function pullFromCarma() {
  ...
  $.ajax({
    ...
    url:
      proxy +
      'http://carma.org/api/1.1/searchLocations?name=' +
      geo_info_object.state,
    method: 'get',
    ...
    success: function(data) {
      ...
      for (var i = 0; i < addressComponentArray.length; i++) {
        for (var j = 0; j < addressComponentArray[i].types.length; j++) {
          if (
            addressComponentArray[i].types[j] === 'administrative_area_level_1'
          ) {
...
function initMap(lat, lng) {
  if (lat === null || lng === null) {
    $('#map_display')
      .text('Location does not exist! Please search again.')
      .addClass('displayMapError');
    return;
  }
...
function getAqiData(keyword) {
  $.ajax({
    ...
    dataType: 'json',
    url:
      '//api.waqi.info/search/?token=' +
      '1af10262d0228050ee6334c5273af092b068ca53' +
      '&keyword=' +
      keyword +
      ',USA',
    success: function(result) {
      if (result.data.length === 0) {
        ...
        $('#aqiNum').text('N/A');
        $('.h_implications').text(
          'No health implications at this time, please try again later.'
        );
        $('.c_statement').text(
...
function getDataByLocation(lat, lon) {
  $.ajax({
    ...
    dataType: 'json',
    url:
      '//api.waqi.info/feed/geo:' +
      lat +
      ';' +
      lon +
      '/?token=1af10262d0228050ee6334c5273af092b068ca53',
    success: function(result) {
...
function getNewsData() {
  if (geo_info_object.lat === null || geo_info_object.lon === null) {
    $('.newsListDisplay').text(
      'Sorry, no articles available for entered location. Please enter a valid city!'
    );
    return;
  }
  ...
  for (var i = 0; i < newsOptions.length; i++) {
    $.ajax({
      url:
        '//newsapi.org/v2/everything?sources=' +
        newsOptions[i] +
        '&q=' +
        cityName +
        '+climate&apiKey=626bed419f824271a515c974d606275b',
      success: function(data) {
...
function displayNewsData(data, newsAvailability) {
  if (newsAvailability === 4) {
    $('.newsListDisplay').text(
      'Sorry, no articles available for entered location. Please enter a different city name!'
    );
    return;
  }
  ...
  for (var newsIndex = 0; newsIndex < data.articles.length; newsIndex++) {
    newsInfo = {
      ...
      imgSource:
        data.articles[newsIndex].urlToImage &&
        data.articles[newsIndex].urlToImage.replace(/^http:/, 'https:'),
    ...
    if (newsInfoArray.length > 1) {
      if (
        newsInfoArray[newsInfoArray.length - 1].newsTitle === newsInfo.newsTitle
      ) {
        newsIndex++;
      } else {
  ...
  for (
    var newsInfoArrayIndex = 0;
    newsInfoArrayIndex < newsInfoArray.length;
    newsInfoArrayIndex++
  ) {
    newsAuthorDiv = $('<div>', {
      class: 'newsAuthor',
    ...
    newsItems = $('<div>')
      .addClass('newsItem')
      .append(newsLinkTag, newsAuthorDiv, newsSourceDiv);
  ...
  function displayModal() {
    ...
    (function() {
      for (
        var newsClickIndex = 0;
        newsClickIndex < newsInfoArray.length;
        newsClickIndex++
      ) {
        ...
        if (
          $(event.target).parent()[0].indexPosition === newsClickIndex &&
          $(event.target).parent()[0].newsSource ===
            newsInfoArray[newsClickIndex].newsSourceID
        ) {
